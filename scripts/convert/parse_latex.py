...
def convert_ref_to_code(node: Macro):
...
def parse_dep_graph(document: TeXDocument) -> list[Node]:
    ...
    for graph in graphs:
        for node_env in graph.nodes:
            ...
            if len(lean_decls) > 1:
                loguru.logger.warning(f"Multiple Lean names found for {node_env.id}: {lean_decls}; only using the first one.")
            ...
            if name in name_to_node:
                loguru.logger.warning(f"Lean name {name} occurs in blueprint twice; only keeping the first.")
